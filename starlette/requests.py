...
def cookie_parser(raw: list[str]) -> dict[str, str]:
    """
    Parse ``Cookie`` HTTP headers into a dict of key/value pairs.

    Accepts a list of cookie header strings (e.g. multiple "cookie" headers in HTTP/2).
    The headers are joined into a single cookie string before parsing.

    This function attempts to mimic browser cookie parsing behavior: browsers and
    web servers frequently disregard the spec (RFC 6265) when setting and reading
    cookies, so we attempt to handle the most common scenarios here.

    This implementation has been adapted from Django 3.1.0.
    Note: we are explicitly *not* using ``SimpleCookie.load`` because it is based
    on an outdated spec and will fail on many inputs we want to support.
    """
    raw_str = "; ".join(value.rstrip("; ") for value in raw if value)
...
class HTTPConnection(Mapping[str, Any]):
    ...
    def cookies(self) -> dict[str, str]:
        if not hasattr(self, "_cookies"):
            ...
            if cookie_headers:
                cookies = cookie_parser(cookie_headers)
